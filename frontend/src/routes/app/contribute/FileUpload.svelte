    ...
    <div class="grid glass p-5 gap-3">
        <div class="p-3 glass grid text-center grid-cols-[1fr_50px_1fr] gap-10 items-center justify-center">
            <div class="grid gap-3">
                ...
                <div class="flex items-center justify-center"><FileSelector text="Select files" multiple={true} bind:files={$filesStore} accept=".txt,.text,.md,.markdown,.qmd,.rmd,.pdf,.docx,.pptx,.xlsx,.xlsm,.html,.htm,.epub,.wav,.mp3,.vtt,.webvtt,.eml,.msg,.png,.jpg,.jpeg,.tif,.tiff,.bmp,.webp,.tex,.latex,.odt,.ods,.odp,.xbrl,.xml,.json,.yaml,.yml" /></div>
            </div>
            ...
            <div class="grid gap-3 items-center m-auto">
                ...
                <a target="_blank" href="../template.xlsm" class="text-nowrap glass px-2 py-1 flex gap-3 items-center justify-center hover:brightness-[90%] cursor-pointer bg-secondary-1/10 text-secondary-1 border-secondary-1">
                ...
                <FileSelector class="w-full" text={$templateStore?.item(0)?.name ?? "Upload CSV file"} multiple={false} bind:files={$templateStore} accept=".csv" />
