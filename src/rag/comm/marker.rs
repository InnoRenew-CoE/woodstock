...
use reqwest::blocking::{multipart as blocking_multipart, Client};
...
impl MarkerClient {
    ...
    pub fn convert_file<P: AsRef<Path>>(&self, file_path: P, options: &ConvertOptions) -> anyhow::Result<ConvertResponse> {
        ...
        let file_name = path.file_name().and_then(|s| s.to_str()).unwrap_or("input.bin").to_string();
